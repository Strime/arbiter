...
- **Cache discipline**: `OffCache` namespaced `arbiter.off-cache`, TTL 7 days, in-flight dedup via `inflight` map in `CompositeManufacturingOriginRepository`
...
9. **Cache invalidation**: on any change to `OffToManufacturingOriginMapper` or `OffCache` shape, bump a version suffix on the cache namespace (`arbiter.off-cache.v2`) to invalidate stale entries.
