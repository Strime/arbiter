...
const profile = mkdtempSync(join(tmpdir(), 'coquade-e2e-'));
...
for (const t of TARGETS) {
  ...
  const logs = [];
  page.on('console', (m) => {
    if (m.text().includes('[coquade]')) logs.push(m.text());
  });
  try {
    ...
    const badges = await page.evaluate(() => document.querySelectorAll('coquade-badge-host[data-coquade-badge]').length);
